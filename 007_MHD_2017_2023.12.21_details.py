...
folder_output = '08_MHD2017_details'

filenames = os.listdir(folder_input)
for filename in filenames:

  filename1 = filename.replace('_stanza_nopunct.txt', '_mhd.txt')
    ...
    if marker == 'EOS':
      ...
      try:
        ...
        for i, (count, indices) in enumerate(level_counts):
          hd_sum += (i + 1) * count    #第一层为1层的话，需要i+1
          dependency_relation += count
        
        if not dependency_relation == 0:
          mhd_2017_sentence = hd_sum/dependency_relation
        else:
          mhd_2017_sentence = ''
        
  #      if not dependency_relation == 1:
  #        mhd_2017_sentence = hd_sum/(dependency_relation - 1)  #计算MHD ；这里不应该减1.也就是说，此时的SSCV就是MHD2017
  #      else:
  #        mhd_2017_sentence =  ''
...
#          sscv_sentence,
        ...
        for i, (count, indices) in enumerate(level_counts):
          ...
          print(f"Level {i + 1}: {count} nodes - {indices_str}", sep = '\t',file = outputfile1)    #第一层为1层的输出形式
        
        # 输出最多节点数的层级和节点数
        print(f"Max nodes: Level {max_node_level} - {max_node_count} nodes", sep = '\t',file = outputfile1)    #第一层为1层的输出形式
        
        # 输出最大深度
        max_depth = get_max_depth(syntax_tree)
        print(f"Max depth: {max_depth}", sep = '\t',file = outputfile1)    #第一层为1层的输出形式
